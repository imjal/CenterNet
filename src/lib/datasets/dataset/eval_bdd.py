# file from https://github.com/ucbdrive/bdd100k/blob/f0a731a32b02b946be06e7891accc95c361ef7f1/bdd100k/evaluate.py
import argparse
import json
import os
...
def cat_pc(gt, predictions, thresholds, center_thresh):
    """
    Implementation refers to https://github.com/rbgirshick/py-faster-rcnn
    """
    image_gts_all = group_by_key(gt, 'name')
    image_pred = group_by_key(predictions, 'name')
    image_gts = { key: image_gts_all[key] for key in image_pred.keys() }
    flat_img_gt = [x for key, value in image_gts.items() for x in value]
    num_gts = len(flat_img_gt)
    ...
    for i in range(len(predictions)):
        if predictions[i]['score'] >= center_thresh:
...
def evaluate_detection(gt, pred, center_thresh):
    ...
    for i, cat in enumerate(cat_list):
        if cat in cat_pred:
            r, p, ap = cat_pc(cat_gt[cat], cat_pred[cat], thresholds, center_thresh)
            aps[:, i] = ap
